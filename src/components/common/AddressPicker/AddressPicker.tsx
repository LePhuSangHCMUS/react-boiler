...
const AddressPicker = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    const getDistrict = async () => {
      axios
        .get(districtURLDefault + province)
        .then((responses) => {
        ...
        <Col sm={24} md={12}>
          <Select
            ...
            {provinceList.map((element, index) => {
              return (
                <Option
                  value={element[provincePropertyDefault?.id]}
                  key={index}>
                  {element[provincePropertyDefault?.name]}
                </Option>
              );
            })}
          </Select>
        ...
        <Col sm={24} md={12}>
          <Select
            ...
            {districtList.map((element, index) => {
              return (
                <Option
                  value={element[districtPropertyDefault?.id]}
                  key={index}>
                  {element[districtPropertyDefault?.name]}
                </Option>
              );
            })}
          </Select>
